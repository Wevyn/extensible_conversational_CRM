...
import openApiSpec from '@/lib/attio_cleaned_no_country_code.json';

class AttioCRMProcessor {
  
  constructor(attioApiKey, groqParam, openApiSpec = null) {
    this.attioApiKey = attioApiKey;
  
    // Secure path: accept a function (proxy fetch) OR a raw key for server-side use
    if (typeof groqParam === 'function') {
      this.groqFetch = groqParam;    // client will call /api/groq/proxy
      this.groqApiKey = null;
    } else {
      this.groqApiKey = groqParam;   // back-compat: direct key (server-only)
      this.groqFetch = null;
    }
...
Respond with ONLY a JSON object like this:
{
  ...
  async callGroq(prompt, options = {}) {
    const payload = {
      ...
      messages: [
        { role: 'system', content: 'You are an expert CRM automation system. Always respond with valid JSON.' },
    ...
    // Secure path: use the proxy function if provided (client → /api/groq/proxy → Groq)
    if (typeof this.groqFetch === 'function') {
      const data = await this.groqFetch('chat/completions', payload);
      return data?.choices?.[0]?.message?.content;
    }
  
    // Back-compat (server only): use a raw key if present
